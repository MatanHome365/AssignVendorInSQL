...
def lambda_handler(event, lambda_context):
    ...
    print(logObject('[INFO]: Complete Lambda Process for project source key : %s' % event['source_key'], 'lambda_handler', 'Complete Lambda'))
...
def assign_vendor(key, text, confidence=0.7, multi_thresh=10, gap_thresh=0.3):
    ...
    # if new_video.json_predictions['probabilites'][new_video.json_predictions['best']] >= confidence or list_of_predictions[0][1] >= list_of_predictions[1][1] * multi_thresh or list_of_predictions[0][1] - gap_thresh >= list_of_predictions[1][1]:
        ...
        print(logObject('[INFO]: A valid prediction has been found: %s' % (new_video.category), 'assign_vendor', 'Found Category'))
    ...
    if is_test == 'False':
        ...
        category_id = load_category_id(conn, new_video.category)

        assigned = assign_project_to_pro(new_video, category_id.lower())
        if assigned == False:
            return None
        if assigned == True:
            # pass
...
def get_incident_id(key):
    ...
    if incident.iloc[0]['project_status_string'] not in not_assigned_statuses:
        ...
        incident['project_id'].values[0], incident['project_number'].values[0], incident['presented_address'].values[0],
...
def find_vendors(data):
    ...
    new_data["category"] = new_data["category"].replace("Garage Door Installer Repair", "Garage Door Installer / Repair")
    conn = psycopg2.connect(host=HOST,
                                database=DBNAME, user=USER, password=PASSWORD)
...
def send_email_to_vendor(new_video):
    ...
    template = {"from":"support@home365.co","subject":"Project assigned","recipients":[{"name":new_video.vendor['Vendor'],"email":new_video.vendor['Email']}],"contentTemplate":{"PRO_NAME": new_video.vendor['Vendor'], "PROJECT_NUMBER": str(new_video.project_number), "PRO_CATEGORY": new_video.category, "ADDRESS":new_video.address, "CHAT_URL": new_video.video_url},"templateName":"Home - Pro - New Project"}
